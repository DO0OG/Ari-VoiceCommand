...
通常の `setup.bat` は、メインアプリと一般的なオプション依存関係用の `.venv` を作成します。
ローカルの CosyVoice3 も準備する場合は、`setup.bat --with-tts` を実行してください。
このコマンドは、CosyVoice3 の CUDA 対応 torch と TTS パッケージ用に2つ目の環境
`.venv-tts` も作成します。この2つの venv を分離する構成により、TTS パッケージが
メインアプリの CPU torch 依存関係を上書きすることを防ぎます。
...
| **リモート操作** | 許可された Telegram チャットから、ローカル UI と同じコマンド処理フローで Ari を遠隔操作できます。 |

### キャラクターウィジェットの主な拡張

- **夜間の眠気モード:** 夜の時間帯にはアニメーション速度を落とし、あくびや眠そうな反応を見せます。
- **プラグイン拡張性:** トレイメニュー、オーバーレイ、音声コマンド、キャラクター反応はマーケットプレイス配布のプラグインで拡張できます。
...
- **Telegram リモートコマンドブリッジ:** 許可リストによる chat_id 認証、long-polling、メッセージ編集によるストリーミング、スクリーンショット写真送信をサポートします（`telegram_enabled`、デフォルト無効）。
- **生成画像ダウンロードの制限:** 画像生成ツールは HTTPS URL からのみ画像をダウンロードするようになりました。
- **自律エージェントの高度な機能:** ローカル MCP サーバー（ファイル読み書きツールを含む）、ストリーミング・ビジョン・ファイル/アプリツール、中断/再開、監査ログ、エージェントダッシュボードを追加しました。
- **多言語コマンドルーティング:** LLMRouter・WeatherCommand・ツールハンドラが韓国語・英語・日本語のキーワードを認識し、言語設定にかかわらずエージェントが正しく起動します。
- **応答キャッシュの外部設定化:** LLM 応答キャッシュの TTL と最大サイズを `ari_settings.json` で調整できます（`agent_response_cache_ttl`、`agent_response_cache_max_size`）。
- **非同期エージェント作業キュー:** `AgentTaskQueue` が優先度付きバックグラウンドタスクスケジューリングとタスク単位のキャンセルを提供します。
- **エージェント結果メッセージの i18n 完成:** 実行ステータス・エージェント要約・レポート場所の文字列が韓国語・英語・日本語すべてで正しく翻訳されます。
- **safety_checker の細分化:** `curl`/`wget` が DANGEROUS から CAUTION に再分類され、エージェントが読み取り専用の HTTP リクエストを送れます（データ送信フラグは DANGEROUS のまま）。
- **フォールバックアシスタントの i18n:** `SimpleAIAssistant` の応答がランタイム翻訳を使用し、Groq 初期化失敗時も正しい言語で応答します。
- **CommandResult の伝播:** `WeatherCommand` などが `CommandResult` を返すようになり、プラグインイベントに正確な成否情報が反映されます。
...
- **i18n 保守の一貫性:** 新規文字列を韓国語・英語・日本語 locale にそろえて反映します。
