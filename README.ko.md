...
- **야간 졸음 모드:** 밤 시간대에는 애니메이션 속도를 늦추고 하품/졸림 반응을 보여줍니다.
- **플러그인 확장 여지:** 트레이 메뉴, 오버레이, 음성 명령, 캐릭터 반응은 마켓플레이스 플러그인으로 확장할 수 있습니다.

---

## 🚀 개발자 관점 하이라이트

- **Python + PySide6 데스크톱 앱:** Windows 앱 구조를 쉽게 파악하고 확장·패키징할 수 있는 형태입니다.
...
- **에이전트 결과 메시지 i18n 완성:** 실행 상태, 에이전트 요약, 보고서 위치 문자열이 한국어·영어·일본어에서 모두 올바르게 번역됩니다.
- **safety_checker 세분화:** `curl`/`wget`이 DANGEROUS에서 CAUTION으로 재분류되어 에이전트가 읽기 전용 HTTP 요청을 할 수 있습니다 (데이터 전송 플래그는 DANGEROUS 유지).
- **폴백 어시스턴트 i18n:** `SimpleAIAssistant` 응답이 런타임 번역을 사용해 Groq 초기화 실패 시에도 올바른 언어로 응답합니다.
- **CommandResult 전파:** `WeatherCommand` 등 명령이 `CommandResult`를 반환해 성공/실패 정보가 플러그인 이벤트에 정확히 반영됩니다.
- **동일 실행 내 즉시 복구:** 실패한 실행은 reflection lesson을 같은 orchestration 세션의 1회 재시도 컨텍스트에 바로 주입할 수 있습니다.
- **백그라운드 reflection 경로:** 성공한 실행은 reflection을 비동기로 예약해 사용자에게 보이는 완료 응답을 지연시키지 않습니다.
- **계획 반복 횟수 동적화:** 고정 루프 횟수 대신 목표 난이도를 추정해 재계획 최대 횟수를 조절합니다.
- **lift 기반 활성화 제어:** 학습 지표에서 효과가 음수로 확인된 컴포넌트는 일시적으로 비활성화할 수 있습니다.
- **i18n 유지보수 일관성:** 신규 문자열을 한국어/영어/일본어 locale에 함께 반영합니다.
