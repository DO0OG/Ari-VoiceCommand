...
- **Windows 설치 파일:** 이제 릴리스에는 `Ari-Setup-<버전>.exe` 하나만 올라갑니다. 기본 설치 위치는 Program Files이고 다른 폴더를 고를 수도 있으며, 설정과 기록은 `%AppData%\Ari`에 저장됩니다.
- **간단한 명령의 빠른 로컬 처리:** 현재 시각, 실행 중인 앱, 스크린샷, 음량 조절 같은 짧은 요청은 LLM을 부르지 않고 기기 안에서 바로 처리할 수 있습니다. 확신이 낮거나 부정하는 말이거나 여러 동작이 섞인 요청은 지금처럼 대화 경로로 넘어갑니다. 기본값은 꺼져 있으며 설정 → 에이전트 → "빠른 로컬 처리"에서 켤 수 있습니다. 릴리스는 평가 문장을 사람이 검수한 뒤에만 만들어집니다.
- **OpenAI 호환 제공자 직접 추가:** 설정 → AI 설정에서 이름과 기본 URL을 등록하고 기본 모델과 API 키(선택)를 넣으면 LM Studio, vLLM, 사내 게이트웨이 같은 OpenAI 호환 서버를 제공자로 쓸 수 있습니다. 키는 암호화 저장소에만 보관됩니다.
- **전원 명령 안전성 강화:** 사용자가 직접 요청했을 때만 컴퓨터를 끄며, "끄지 마"처럼 부정하는 말로는 절대 실행되지 않습니다. 모델의 추론 문장을 그대로 읽어 주던 문제도 고쳤습니다(NVIDIA Nemotron의 추론 모드를 끕니다).
