...
- **Timer**: `TimerManager.set_timer(minutes: float)` — single `threading.Timer(total_seconds, cb)`. `parse_timer_command` uses regex per unit (시간/분/초) summed, returns float minutes. Matches "타이머" or "알람".
