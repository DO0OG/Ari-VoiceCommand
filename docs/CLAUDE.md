...
- **Validate**:
  ...
  - Tests: `py -3.11 -m unittest VoiceCommand.tests.test_agent_integration VoiceCommand.tests.test_agent_planner_parsing VoiceCommand.tests.test_episode_memory VoiceCommand.tests.test_llm_provider VoiceCommand.tests.test_real_verifier VoiceCommand.tests.test_validate_repo VoiceCommand.tests.test_cosyvoice_tts VoiceCommand.tests.test_autonomous_executor VoiceCommand.tests.test_ai_command VoiceCommand.tests.test_automation_helpers` (총 156개)
...
2. `git status` — 로컬 변경 파악 (`ari_settings.json`·런타임 산출물은 커밋 제외)
...
- **Agent System**:
  - `AgentOrchestrator`: Multi-step task execution with parallel group scheduling and DOM replanning. Post-run calls `_post_run_update()` → updates StrategyMemory, SkillLibrary, PlannerFeedback, UserProfile, MemoryIndex, EpisodeMemory (+ `prune_old_failures(30일)`). Developer goal 시 step 출력 저장 한도 2000자 (일반 목표 300자) — bootstrap JSON이 잘리지 않도록.
  - `AgentPlanner`: Template-first planning → DAG annotation (`dag_builder.py`). Accepts few-shot examples + planner hints injected in system prompt. Developer goal bootstrap: step_0(repo scan, samples=5), step_1(validate_repo.py 경로), step_2(test_*.py 목록). `_infer_relevant_tests(test_names, goal)` — goal에서 `.py` 파일명 추출해 관련 테스트 우선 표시.
...
- **Bootstrap step sizes** (2026-04-02 기준): step_0 ~954자, step_2 ~783자 (test_*.py 29개). 모두 2000자 이내.
- **Commit exclusions**: `VoiceCommand/ari_settings.json`, `ari_memory.db`, `episode_memory.json`, `planner_stats.json`, `skill_library.json`, `user_profile.json`, `plugins/*.zip`, `collected.json`, `AGENT_HANDOFF_*.md`, `.idea/`.
