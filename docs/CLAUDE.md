...
- **Validate**:
  ...
  - Tests: `py -3.11 -m unittest discover -s VoiceCommand/tests -p "test_*.py"` (총 348개)
...
3. 검증: `validate_repo.py --compile-only` → 영향 테스트 → `git add` (로컬 파일 제외)
...
- **Agent Skills / MCP**:
  - `SkillManager` (`agent/skill_manager.py`): `skills/` 디렉터리에서 `SKILL.md`를 로드하고, frontmatter / trigger keywords / MCP endpoint를 파싱합니다. `build_match_context()`가 스킬 프롬프트, `required_tool_names`, `preferred_tool`을 제공합니다.
  - `SkillInstaller` (`agent/skill_installer.py`): 로컬 디렉터리, GitHub tree URL, HTTPS URL에서 스킬을 설치합니다. `.ari_skill_meta.json`에 원본과 활성화 상태를 저장하고, 업데이트 시 백업·복원합니다.
  - `McpClientPool` / `McpSession` (`agent/mcp_client.py`): MCP Streamable HTTP 세션을 관리합니다. `initialize → notifications/initialized → tools/call → close` 흐름과 SSE 응답 파싱을 처리합니다. HTTPS만 허용합니다.
  - `AICommand._handle_mcp_call()`: `mcp_call(endpoint, tool, arguments)` 내장 도구를 실제 MCP 호출로 연결합니다.
  - `ui/skills_dialog.py`: 스킬 관리 창. 트레이 메뉴와 설정창에서 진입합니다.
  - `LLMProvider._get_skill_context()`: 사용자 요청과 매칭된 스킬을 시스템 프롬프트에 주입합니다.
