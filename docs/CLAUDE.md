...
- **Validate**:
  ...
  - Tests: `py -3.11 -m unittest discover -s VoiceCommand/tests -p "test_*.py"` (총 268개)
...
2. `git status` — 로컬 변경 파악 (특히 `VoiceCommand/.ari_runtime/`·런타임 산출물은 커밋 제외, 루트 `ari_settings.json`은 템플릿 기준선 유지)
...
- **Text streaming**: `ui/text_interface.py` — `stream_chunk` signal updates the visible response immediately, and `_try_stream_tts()` starts TTS on sentence boundaries before the full response finishes. `test_text_interface.py` covers stream-TTS flush behavior.
...
- **Commit exclusions**: `VoiceCommand/.ari_runtime/`, `ari_memory.db`, `episode_memory.json`, `planner_stats.json`, `skill_library.json`, `user_profile.json`, `learning_metrics.json`, `plugins/*.zip`, `collected.json`, `AGENT_HANDOFF_*.md`, `.idea/`.
