...
캐릭터 위젯은 마켓플레이스 플러그인을 통해 상태 반응형 UI로 확장할 수 있습니다.

예를 들어 아래와 같은 기능을 플러그인으로 제공할 수 있습니다.
...
- 메인 저장소에는 이러한 기본 플러그인 구현을 포함하지 않습니다.
- 필요한 기능은 마켓플레이스 ZIP을 내려받아 `%AppData%\Ari\plugins` 또는 `VoiceCommand\.ari_runtime\plugins`에 추가해 사용합니다.
- 플러그인 설치 후에는 트레이 메뉴/캐릭터 메뉴/명령 체계에 해당 기능이 자동 등록될 수 있습니다.
...
- 다만 이 모드는 자율 실행 엔진 자체를 켜고 끄는 스위치가 아닙니다. 모드가 꺼져 있어도 명시적인 다단계 요청이나 LLM의 직접 tool call로 에이전트가 실행될 수 있습니다.
...
타이머 명령은 한국어·영어·일본어 시간 단위를 모두 인식합니다. 언어 설정과 무관하게 세 언어를 혼용해도 파싱됩니다.
...
- 단계별 상태 변화(활성 창, URL, 새 창, 새 파일)를 기록합니다.
...
- 파일 삭제·레지스트리·포맷 등 **위험 작업**이 감지되면 `SafetyChecker`가 위험 등급을 평가하고, 위험 수준에 따라 **위험 작업 확인 다이얼로그**(15초 자동 취소)를 표시합니다. 다이얼로그에는 감지된 위험 패턴 목록과 요약이 함께 제공됩니다.
- 긴 자율 작업은 `AgentTaskQueue`를 통해 백그라운드에서 우선순위 기반으로 처리됩니다. 높은 우선순위 작업은 큐 안에서 먼저 실행되며, 개별 작업 단위로 취소 신호를 보낼 수 있습니다.
...
메인 저장소는 기본 플러그인을 함께 번들하지 않습니다. 플러그인은 마켓플레이스 ZIP을 통해 별도 배포하고, 사용자가 런타임 플러그인 폴더에 직접 추가하는 방식을 기준으로 합니다.
...
아래 키는 `VoiceCommand/.ari_runtime/ari_settings.json`(소스 실행) 또는 `%AppData%\Ari\ari_settings.json`(빌드된 exe)에서 직접 편집할 수 있습니다. 초기값은 `VoiceCommand/ari_settings.template.json`을 참고하세요.

### LLM 응답 캐시

| 키 | 기본값 | 설명 |
|----|--------|------|
| `agent_response_cache_ttl` | `600` | 응답 캐시 유효 시간(초). 같은 요청에 대한 LLM 호출을 이 시간 동안 재사용합니다. |
