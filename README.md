...
> Windows 전용 다국어(한국어·영어·일본어) 음성 AI 어시스턴트.
...
처음 읽는 사람이 한 번에 파악하기 쉽도록 `입력/대화 → 실행/자동화 → 학습/기억 → 안전/확장 → 운영/배포` 순서로 정리했습니다.
...
| 기능 | 설명 |
|------|------|
| **스마트 어시스턴트 모드** | 복합 요청을 `run_agent_task`로 더 적극적으로 자동 승격하고, 사용자 패턴 기록을 함께 활성화 |
...
| **런타임 분리 저장** | 소스 실행은 `VoiceCommand/.ari_runtime/`, 빌드된 exe 실행은 `%AppData%/Ari/` 사용 |
| **검증 스크립트** | `validate_repo.py`가 compile + unittest + clean runtime smoke + marketplace contract smoke를 함께 확인 |

---

## 처음 읽는 순서

1. 설치와 실행부터 보려면 아래 [빠른 시작](#빠른-시작)
2. 자동화 동작 방식을 이해하려면 [스마트 어시스턴트 모드](#스마트-어시스턴트-모드)
3. 학습이 실제로 어떻게 좋아지는지 보려면 [자율실행 성공률과 자가학습 진행도](#자율실행-성공률과-자가학습-진행도)
4. 세부 사용법은 [문서](#문서)와 [설정](#설정)
5. 내부 구조를 보려면 [아키텍처](#아키텍처)
...
- 스마트 어시스턴트 모드는 **자율실행 엔진 자체를 켜고 끄는 스위치가 아닙니다.**
- 실제 자율실행 엔진은 `AICommand -> run_agent_task -> AgentOrchestrator.run()` 경로로 동작합니다.
- 이 모드를 켜면:
  - 복합 요청이 `run_agent_task`로 자동 승격될 가능성이 높아지고
  - 사용자 요청 패턴 기록이 더 적극적으로 쌓입니다.
- 이 모드를 꺼도:
  - LLM이 직접 `run_agent_task`를 호출하거나
  - 명시적으로 다단계 작업을 요청하면
  - 자율실행 엔진은 여전히 실행될 수 있습니다.
- 즉, 이 모드는 “에이전트 실행 허용 여부”보다는 “자동 승격과 학습을 얼마나 공격적으로 할지”에 가까운 설정입니다.
...
<details>
<summary>최근 업데이트 자세히 보기 (2026-03-29 ~ 2026-04-06)</summary>

### 최근 업데이트 (2026-04-06) — 실행 엔진 조건 평가 분리 + 문서 최신화
...
- **설정 템플릿 분리**: 실제 설정 파일 대신 `VoiceCommand/ari_settings.template.json`만 저장소에 유지하고, 실사용 설정은 런타임 경로에 생성되도록 정리.
- **빌드 안전장치 추가**: `build_exe.py`가 템플릿 내 민감 설정 키를 검사하고, 실제 `ari_settings.json`이 아니라 템플릿만 포함하도록 수정.
- **마켓플레이스 설치 수 동기화**: `record_plugin_install` SQL 함수 추가로 `install_count`를 installs 기준으로 동기화.
- **업로드 제한 강화**: 5MB 업로드 제한, ZIP 엔트리 수/압축 해제 총량 검사, 검증 파이프라인 시작 실패 시 rejected 상태 반영.
- **프런트엔드 개선**: 업로드 상태 폴링을 단일 `plugin-status` 함수로 분리하고, 설치 추적 helper를 `src/lib/pluginInstall.ts`, 업로드 크기 검사를 `src/lib/pluginUpload.ts`로 분리.
- **런타임/성능 개선**: `ResponseCache` 분리, weather TTL 캐시 + `requests.Session()` 재사용, ElevenLabs 스트리밍 수신 경로 개선, CosyVoice 설치 대기 루프 안정화.
...
- **Python 코드 컴파일 (Direction 2)**: `SkillOptimizer.compile_to_python()` — 5회 성공한 검증된 스킬을 단일 `run_skill()` Python 함수로 컴파일하고 `VoiceCommand/.ari_runtime/compiled_skills/`에 저장. 이후 실행 시 LLM 계획 없이 Python 직접 호출.
...
사용자 요청
 ...
 LLMProvider (+ ResponseCache · LLMRouter · UserProfile · tool_schemas)
     ...
     ├── 단순 도구 호출 ──────────────────────────────────► 즉시 실행
     │
     └── run_agent_task (다단계 목표, 또는 스마트 어시스턴트 모드에서 자동 승격)
              │
              ▼
